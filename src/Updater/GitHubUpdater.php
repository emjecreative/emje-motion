...
final class GitHubUpdater
{
    ...
    private function getReleaseInfo(): ?array
    {
        ...
        if (is_array($cached) && isset($cached['version'], $cached['download_url'], $cached['body'], $cached['published_at'])
            && is_string($cached['version']) && is_string($cached['download_url'])
            && is_string($cached['body']) && is_string($cached['published_at'])) {
            // SECURITY: re-validate the cached download host at use-time so a
            // poisoned transient can never point the updater at an arbitrary URL.
            if ($this->isAllowedDownloadHost($cached['download_url'])) {
                return [
