...
export default class LenisScroll {
    constructor(config = {}) {
        const lerp = Number(config.lerp ?? 0.075);
        const wheel = Number(config.wheelMultiplier ?? 1.2);
        this.config = {
            lerp: Math.min(0.2, Math.max(0.05, Number.isFinite(lerp) ? lerp : 0.075)),
            wheelMultiplier: Math.min(1.5, Math.max(0.8, Number.isFinite(wheel) ? wheel : 1.2)),
    ...
    bindAnchors() {
        ...
        document.addEventListener('click', this._anchorHandler);
    }
    ...
    destroy() {
        ...
        if (this._anchorHandler) {
            document.removeEventListener('click', this._anchorHandler);
            this._anchorHandler = null;
        }
