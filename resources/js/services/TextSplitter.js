...
export default class TextSplitter {
    ...
    constructor(element) {
        ...
        this.originalHTML = sanitizeHtml(element.innerHTML);
	...
	splitTextNode(node, className, characters) {
		...
		parts.forEach((part) => {
			...
			if (characters) {
				...
				// SECURITY: never re-parse text via innerHTML. A literal
				// `&lt;img/src=x/onerror=...&gt;` in the source text decodes
				// to a live element when parsed as HTML → stored XSS.
				// Use textContent with \u00A0 to keep word gaps visible.
				wrapper.textContent =
					part.replace(/ /g, '\u00A0');
